...
| `CH7` | `NC` | не используется; PB4/K1 удалены |
...
Текущая сборка не содержит K1 и его драйвер. `PB4`, `PB1` и STEVAL `J2-21`
оставить `NC`; `PB4` программно переведён в высокоомный режим.

`BR1.PLUS -> HV_DC+` разрешается только через отдельный рассчитанный сетевой
ограничитель пускового тока до выпрямителя. Прямое включение накопительного
конденсатора в сеть запрещено. См. `POWER_INPUT_NO_RELAY_RU.md`.
...
- подключение PB4, PB1 или STEVAL J2-21: все три контакта должны оставаться `NC`.
...
3. Проверить, что K1, его драйвер и провод PB4 физически отсутствуют, а внешний
   ограничитель пускового тока установлен до выпрямителя согласно паспорту устройства.
