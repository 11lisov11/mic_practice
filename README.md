...
В `LV TEST` необходимо ввести `ARM LV HV OFF`. Профиль допускается только при физически отключённой силовой шине и телеметрии `0..10 В`; каждый `START` автоматически ограничивается 3 секундами. В `HV 310 В` требуется `ARM 310V`, а телеметрия шины должна находиться в диапазоне `100..400 В`. Разрешение выдаётся на 30 секунд только при живой связи с Nucleo, состоянии `SAFE`, выключенном PWM и K1, отсутствии `ESTOP`, fault и ошибок UART, свежей телеметрии и правдоподобной температуре радиатора. После `START` Nucleo ждёт `Vbus >= 250 В`, включает K1 через `PB4`, выдерживает 350 мс и только затем запускает MCSDK; HMI подтверждает фактические K1 и `PWM_ACTIVE`. Ручная команда `PRECHARGE` запрещена. Это программное ограждение не заменяет внешний E-STOP, аппаратный shutdown IPM, предохранитель, аппаратный резистор предзаряда и закрытый корпус.

Матрица UNO Q показывает заданную частоту ещё до запуска. При работе она чередует частоту и расчётные обороты; нижние три пикселя обозначают экран оборотов. Два пикселя слева показывают фактический K1 от Nucleo, два пикселя справа - фактический `PWM_ACTIVE` MCSDK. На веб-странице частота и обороты отображаются одновременно.
...
- `PRECHARGE ON|OFF` - не поддерживается вручную: K1 автоматически управляется Nucleo через PB4
- `PFC ON|OFF`
...
До `START` поля `precharge` и `bp_ext & 0x08` должны быть нулевыми. Во время запуска и работы Nucleo устанавливает их после достижения порога Vbus; HMI ждёт этот признак вместе с реальным `PWM_ACTIVE`.

Матрица UNO Q показывает заданную частоту ещё до `START`. Верхняя левая точка зарезервирована и всегда погашена, верхняя правая означает активный PWM; после `STOP` она должна погаснуть.
...
Low-voltage regression должен подтверждать `precharge=0` и `(bp_ext & 0x08)=0` до команды запуска. Отдельная HIL-проверка должна подтвердить PB4 LOW после reset/STOP/fault/timeout, порог 250 В, выдержку 350 мс и размыкание K1 до снятия программной блокировки.
...
NTC/PRECHARGE tooling; ручное управление K1 отклоняется, поскольку PB4 принадлежит
автоматическому interlock Nucleo. Тринадцатая
