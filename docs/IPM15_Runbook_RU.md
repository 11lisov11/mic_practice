...
* `UNO Q` (прошивка `UNOQ_MOTOR.ino`): UI по HTTP (`/api/cmd`, `/api/status`), логика режимов, отображение частоты на матрице, обмен с Blue Pill по UART.
...
* В текущей прошивке UNOQ кнопка аварийного останова в HMI работает как быстрый `STOP`, а команда `ESTOP` на UNOQ после остановки автоматически отправляет `CLEAR` в Blue Pill.
