...
Для АИР56B2, соединённого физически в `Delta` с инвертором `220 В line-to-line`, паспортное напряжение одной обмотки равно `220 В`, но в MCSDK V/F должно стоять `127 В phase-to-neutral` (`220 / sqrt(3)`). Это не смена соединения: это эквивалентное представление треугольника внутри трёхфазной модели контроллера. Подготовленный профиль шильдика: `docs/mcsdk_acim_motor_profile.iek_air56b2_nameplate_verified_vf.json`.
...
Так как в `MIC_AI.pdf` предусмотрен внешний precharge-релейный узел, release-gate также требует явного `#define MIC_PRECHARGE_INTERLOCK_IMPLEMENTED 1` в исходниках Nucleo. Его нельзя добавлять как заглушку: перед этим должны быть реализованы реальный выход Nucleo на драйвер реле, задержка до измеренного Vbus, запрет PWM до готовности, размыкание на fault/E-stop/timeout и HIL-проверка. В текущем AIR56B2-проекте такого interlock нет, поэтому он намеренно не пройдёт release-gate.
