...
- Это прошивка-кандидат AIR56B2 для этапа S1 V/F по шильдику. Она не является допуском к силовому запуску: измерения двигателя и автономный сетевой soft-start пока не подтверждены на стенде.
...
Описание кадра и прямого соединения UART: `docs/UNOQ_MCSDK_UART_CONTRACT_RU.md`.
...
5. Адаптер использует прямой `USART1`: `PB6/TX -> UNO D0/RX`, `UNO D1/TX -> PB7/RX`, общая `HOT_GND`. При timeout 300 мс, E-stop или fault он обязан выполнить API-останов MCSDK и потребовать новую явную команду запуска.
...
Дополнительно gate статически проверяет, что `STEVAL J2-1 -> IHM09M2 J7-1` приходит на `PA6/TIM1_BKIN`, break активен низким уровнем и автоматическое восстановление PWM запрещено. Физическое подтверждение принимается только отдельным протоколом `mic_ai.ipm_sd_bkin_hil.v1`, привязанным SHA-256 к конкретному HEX и файлу Saleae. Порядок испытания приведён в `docs/IPM_SD_BKIN_HIL_RU.md`.

Внешний сетевой soft-start устанавливается перед выпрямителем, автономно ограничивает ток четырьмя NTC и шунтирует их собственным внутренним реле. Управляющего GPIO от Nucleo нет, `PB4` не используется, а устаревший UART-бит реле всегда равен нулю. Адаптер разрешает MCSDK только после устойчивого `Vbus 250...385 В` в течение 3,5 с. Исходник содержит `MIC_EXTERNAL_SOFTSTART_CONFIGURED 1`, `MIC_SOFTSTART_GPIO_CONTROLLED 0` и `MIC_EXTERNAL_SOFTSTART_HIL_VALIDATED 0`. Последний признак меняется только после измерения пускового тока, времени шунтирования NTC, устойчивости DC-шины и поведения при повторном включении.

Этапы S1 V/F, S2 LSO-FOC, S3 AS5600 teacher, S4 AI shadow и S5 hybrid описаны в `docs/FIRMWARE_STAGES_RU.md`. До измерения `Rs`, `Rr`, `Lls`, `Llr`, `Lm` и инерции выпуск LSO-FOC-бинарника заблокирован; демонстрационные параметры ST использовать для AIR56B2 запрещено.
