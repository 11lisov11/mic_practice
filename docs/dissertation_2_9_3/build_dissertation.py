...
RESEARCH_MANIFEST = ROOT / "artifacts" / "air56b2_research_manifest.json"
VF_MATRIX = ROOT / "artifacts" / "air56b2_vf_operating_matrix.json"
FAULT_MATRIX = ROOT / "artifacts" / "air56b2_protection_fault_matrix.json"
ENCODER_FOC_STUDY = ROOT / "artifacts" / "air56b2_encoder_foc_fidelity_study.json"
LOSS_STUDY = ROOT / "artifacts" / "air56b2_loss_optimization_study.json"
SENSORLESS_STUDY = ROOT / "artifacts" / "air56b2_sensorless_independent_plant_study.json"
POLICY_BENCHMARK = ROOT / "artifacts" / "air56b2_policy_benchmark.json"
COMMON_CONTROL_BENCHMARK = ROOT / "artifacts" / "air56b2_common_control_benchmark.json"
...
def make_figures() -> dict[str, Path]:
    ...
    for i, ((title, subtitle), x, w) in enumerate(zip(specs, xs, widths)):
        box(
            ...
            title,
            subtitle,
            fill=FIG_BLUE_FILL if i in (0, 1) else FIG_GRAY_FILL,
            outline=FIG_BLUE if i in (0, 1) else FIG_LINE,
        )
        if i < len(specs) - 1:
            arrow(d, (x + w, 400), (xs[i + 1] - 15, 400))
    d.text((300, 560), "Прямой UART: PB7/RX ← UNO D1/TX; PB6/TX → UNO D0/RX; общая HOT_GND", font=load_font(27), fill=FIG_INK)
    ...
    box(d, (120, 180, 790, 780), "Подтверждено программно", f"Числовая согласованность профиля AIR56B2\nMCSDK: успешная сборка\nELF, BIN, HEX и SHA-256\nUART v0x02 и схема соединений\nV/f, fault matrix, encoder FOC\nManifest v6: {gate_count}/{gate_count} ворот", fill=FIG_TEAL_FILL, outline=FIG_TEAL)
    box(d, (1010, 180, 1680, 780), "Требует аппаратной проверки", "Шильдик и параметры экземпляра\nФактическая фазировка и калибровка АЦП\nАппаратный E-stop, break/OCP и HIL\nRs, Rr, Lls, Llr, Lm и J\nSensorless на низкой скорости\nЭкспериментальный выигрыш ИИ", fill=FIG_RED_FILL, outline=FIG_RED)
    ...
    common = read_json(COMMON_CONTROL_BENCHMARK)
    img, d = canvas("Парное сравнение стратегий на общем holdout")
    methods = [
    ...
    x0, x1, y0, y1 = 520, 1670, 180, 780
    d.line((x0, y1, x1, y1), fill=FIG_INK, width=4)
    for tick in range(7):
        value = 3 * tick
        x = x0 + (x1 - x0) * value / 18
        d.line((x, y1, x, y1 + 12), fill=FIG_INK, width=3)
        d.text((x - 12, y1 + 18), str(value), font=load_font(27), fill=FIG_INK)
    row_height = (y1 - y0) / len(methods)
    for index, (key, name) in enumerate(methods):
        y = y0 + row_height * (index + 0.5)
        metrics = common.get("methods", {}).get(key, {}).get("saving_vs_fixed_pct", {})
        mean = float(metrics.get("mean", 0.0))
        median = float(metrics.get("median", 0.0))
        ci = metrics.get("mean_cluster_bootstrap_95_ci", [0.0, 0.0])
        low, high = float(ci[0]), float(ci[1])
        to_x = lambda value: x0 + (x1 - x0) * max(0.0, min(18.0, value)) / 18
        d.text((70, y - 18), name, font=load_font(29), fill=FIG_INK)
        d.line((to_x(low), y, to_x(high), y), fill=FIG_INK, width=5)
        d.line((to_x(low), y - 12, to_x(low), y + 12), fill=FIG_INK, width=3)
        d.line((to_x(high), y - 12, to_x(high), y + 12), fill=FIG_INK, width=3)
        d.ellipse((to_x(mean) - 10, y - 10, to_x(mean) + 10, y + 10), fill="white", outline=FIG_INK, width=4)
        d.line((to_x(median) - 8, y - 9, to_x(median) + 8, y + 9), fill=FIG_INK, width=3)
        d.line((to_x(median) - 8, y + 9, to_x(median) + 8, y - 9), fill=FIG_INK, width=3)
    d.text((760, 850), "Снижение расчётных потерь относительно фиксированного потока, %", font=load_font(28), fill=FIG_INK)
    figs["common_benchmark"] = save(img, "fig13_common_benchmark.png")
...
def add_status_page(doc: Document) -> None:
    ...
    add_table(doc, "Таблица 1 - Статус основных частей исследования", ["Часть", "Статус", "Разрешённая формулировка"], [
        ["Скалярное V/f", "Прошивка + SIL 48/48", "Кандидат S1; требуется стендовая проверка"],
        ["Векторное FOC", "Encoder-observer SIL 24/24", "Перенос после идентификации двигателя"],
        ["ИИ-настройка", "Policy holdout 600 точек", "Расчётный выигрыш; supervisor обязателен"],
...
def add_toc_entry(doc: Document, title: str, page: str, level: int = 0) -> None:
    ...
    set_run_font(r, size=11, bold=(level == 0))
...
def add_toc_and_abbreviations(doc: Document) -> None:
    ...
    set_run_font(r, bold=True)
    entries = [
        ("СПИСОК СОКРАЩЕНИЙ И ОБОЗНАЧЕНИЙ", "4", 0),
        ("ВВЕДЕНИЕ", "5", 0),
        ("1 АНАЛИЗ МЕТОДОВ УПРАВЛЕНИЯ АСИНХРОННЫМ ТЯГОВЫМ ЭЛЕКТРОПРИВОДОМ", "11", 0),
        ("1.1 Требования тягового применения", "11", 1),
        ("1.2 Скалярное управление V/f", "12", 1),
        ("1.3 Векторное управление и идентификация", "13", 1),
        ("1.4 Интеллектуальные методы и требования к доказательству", "13", 1),
        ("1.5 Постановка научной задачи", "15", 1),
        ("1.6 Выводы по главе 1", "15", 1),
        ("2 МАТЕМАТИЧЕСКИЕ МОДЕЛИ И КРИТЕРИИ КАЧЕСТВА", "17", 0),
        ("2.1 Каталожный профиль и расчётная модель двигателя АИР56В2", "17", 1),
        ("2.2 Модель асинхронной машины в координатах dq", "18", 1),
        ("2.3 Скалярный закон, номинальная область и ослабление поля", "19", 1),
        ("2.4 Преобразования Clarke и Park, структура FOC", "20", 1),
        ("2.5 Пространственно-векторная модуляция и ограничения", "21", 1),
        ("2.6 Критерии энергетической эффективности и робастности", "23", 1),
        ("2.7 Граница переноса к тяговому электроприводу", "23", 1),
        ("2.8 Выводы по главе 2", "24", 1),
        ("3 ПРОГРАММНО-АППАРАТНАЯ ПЛАТФОРМА И ПРОСЛЕЖИВАЕМОСТЬ", "25", 0),
        ("3.1 Архитектура стенда", "25", 1),
        ("3.2 Конфигурация MCSDK и двигателя", "26", 1),
        ("3.3 Контракт связи UNO Q — Nucleo", "27", 1),
        ("3.4 Безопасное состояние и аппаратные защиты", "29", 1),
        ("3.5 Сборка, хэширование и управление конфигурацией", "30", 1),
        ("3.6 Готовность к расширению", "31", 1),
        ("3.7 Выводы по главе 3", "31", 1),
        ("4 КОНЦЕПЦИЯ МЕТОДА ОГРАНИЧЕННОЙ ИНТЕЛЛЕКТУАЛЬНОЙ АДАПТАЦИИ FOC", "33", 0),
        ("4.1 Иерархическая организация управления", "33", 1),
        ("4.2 Адаптация параметров модели и регуляторов", "34", 1),
        ("4.3 Перспективное расширение: горизонтный выбор векторов", "35", 1),
        ("4.4 Safety Gateway и проверяемые инварианты", "35", 1),
        ("4.5 Цифровой двойник и цикл обучения", "36", 1),
        ("4.6 Программа сравнения алгоритмов", "38", 1),
        ("4.7 Выводы по главе 4", "39", 1),
        ("5 ПРОГРАММНАЯ ПРОВЕРКА И МЕТОДИКА ЭКСПЕРИМЕНТАЛЬНОГО ИССЛЕДОВАНИЯ", "40", 0),
        ("5.1 Подтверждённые результаты программной проверки", "40", 1),
        ("5.2 Результаты воспроизводимого SIL-исследования", "41", 1),
        ("5.3 Последовательность HIL- и стендовых испытаний", "44", 1),
        ("5.4 План экспериментов и измерительные каналы", "45", 1),
        ("5.5 Статистическая обработка", "46", 1),
        ("5.6 Критерии завершения этапов", "47", 1),
        ("5.7 Выводы по главе 5", "48", 1),
        ("ЗАКЛЮЧЕНИЕ", "49", 0),
        ("СПИСОК ИЛЛЮСТРАТИВНОГО МАТЕРИАЛА", "51", 0),
        ("СПИСОК ЛИТЕРАТУРЫ", "52", 0),
        ("ПРИЛОЖЕНИЕ А Каталожный и расчётный профиль двигателя АИР56В2", "54", 0),
        ("ПРИЛОЖЕНИЕ Б Таблица соединения UART верхнего уровня", "55", 0),
        ("ПРИЛОЖЕНИЕ В Форма протокола стендового испытания", "56", 0),
        ("ПРИЛОЖЕНИЕ Г Чек-лист допуска к силовому запуску", "57", 0),
        ("ПРИЛОЖЕНИЕ Д Матрица трассируемости задач и доказательств", "58", 0),
    ]
...
def add_intro(doc: Document, figs: dict[str, Path]) -> None:
    ...
    paragraphs = [
        ...
        "Исходная инженерная база исследования включает NUCLEO-G431RB на микроконтроллере семейства STM32G431RBTx, переходную плату X-NUCLEO-IHM09M2, силовой модуль STEVAL-IPM15B, асинхронный двигатель IEK АИР56В2 и вычислительный узел Arduino UNO Q. Для этой конфигурации подготовлена сборка официального примера MCSDK ACIM V/f Open Loop, разработан программный контракт прямого UART внутри общего HOT-домена, сформирован комплект ELF, BIN и HEX и выпущена многостраничная схема соединений. Аппаратный UART-тракт, силовые платы и двигатель ещё не прошли совместные стендовые испытания, поэтому текущий результат является программно проверенным кандидатом.",
        "До появления плат выполнен воспроизводимый SIL-этап: сформирован ансамбль из 256 допустимых вариантов неопределённых параметров, проверены V/f и матрица защит, настроен encoder-observer FOC, исследован независимый sensorless-наблюдатель и проведено парное сравнение стратегий управления на 600 отложенных режимах. Эти результаты уточняют научную гипотезу и сокращают объём будущей отладки, но не заменяют идентификацию параметров и аппаратный эксперимент.",
    ]
    for t in paragraphs:
        add_body(doc, t)
...
def add_chapter3(doc: Document, figs: dict[str, Path]) -> None:
    ...
    add_table(doc, "Таблица 3.1 - Основные компоненты платформы", ["Компонент", "Назначение", "Критическое ограничение"], [
        ...
        ["UNO Q", "Верхний уровень и ИИ", "Прямой UART; весь узел внутри HOT-корпуса"],
    ...
    add_body(doc, "Физическое соединение выполняется прямым UART 3,3 В внутри закрытого HOT-корпуса: UNO D1/Serial1 TX соединяется с PB7/USART1_RX, а PB6/USART1_TX — с UNO D0/Serial1 RX. Земли UNO Q и Nucleo объединены с HOT_GND, электрически связанной с J7 DC−; шины питания 3,3 В между платами не соединяются. Параметры UART: 115200 бод, 8 бит данных, без чётности, один стоповый бит. При включённой силовой части наружу выходит только Wi-Fi, а USB, ST-Link и измерительные кабели физически отсутствуют. Таблица соединений приведена в приложении Б.")
    ...
    add_body(doc, "Архивная схема hardware/mic_ai_rev2 относится к Blue Pill и не используется для монтажа. Для NUCLEO-G431RB — IHM09M2 — IPM15B выпущен документ MIC_AI_NUCLEO_SYSTEM_WIRING.pdf с полным соответствием FC-34P, питанием, двигателем, AS5600 и автономным Wi-Fi-контуром. Физическая приёмка монтажа, аппаратного E-stop и силовой части остаётся отдельным HIL-этапом.")
    add_body(doc, "Внешний soft-start автономен и не имеет управляющего выхода Nucleo. Программный запуск разрешается только после подтверждения устойчивой Udc, отсутствия fault/E-stop, валидной температуры, исправной UART-связи и фактического состояния PWM; тайм-ауты защёлкивают остановку. Аппаратная HIL-проверка этих условий ещё не завершена, поэтому release-gate не присваивает статус hardware_validated.")
    ...
    add_body(doc, "Следовательно, на текущем этапе подготовлены программная конфигурация, контракт взаимодействия и выпущенная многостраничная схема соединений Nucleo/IHM09M2/IPM15B/UNO Q, но платформа ещё не проверена физически. Первый аппаратный цикл должен подтвердить ревизии плат, ориентацию 34-контактного кабеля, фактическую распиновку, уровни 3,3 В и отсутствие конфликта UART с отладочными функциями.")
...
def add_chapter5(doc: Document, figs: dict[str, Path]) -> None:
    ...
    research_manifest = read_json(RESEARCH_MANIFEST)
    vf_matrix = read_json(VF_MATRIX)
    fault_matrix = read_json(FAULT_MATRIX)
    encoder_foc = read_json(ENCODER_FOC_STUDY)
    loss_study = read_json(LOSS_STUDY)
    sensorless_study = read_json(SENSORLESS_STUDY)
    policy_benchmark = read_json(POLICY_BENCHMARK)
    common_benchmark = read_json(COMMON_CONTROL_BENCHMARK)
    ...
    doc.add_heading("5.2 Результаты воспроизводимого SIL-исследования", level=2)
    manifest_gates = research_manifest.get("gates", {})
    gate_count = len(manifest_gates)
    add_body(doc, f"Вместо использования архивной host-матрицы сформирован канонический конвейер AIR56B2 с прослеживаемыми входами, фиксированным master seed 560225 и SHA-256 всех результатов. Manifest версии 6 имеет статус {research_manifest.get('status', 'не сформирован')}; пройдено {sum(bool(value) for value in manifest_gates.values())} из {gate_count} программных ворот. Флаг hardware_release_ready остаётся false, поэтому термин PASS далее относится только к SIL-уровню.")
    add_body(doc, "Параметры F1 сформированы из паспортных величин АИР56В2, а неопределённые электрические и механические параметры представлены ансамблем из 256 согласованных F2/F3-вариантов. Эти варианты не объявляются идентифицированными параметрами реального двигателя. Такой подход позволяет проверять чувствительность алгоритмов до получения платы, сохраняя отдельный gate аппаратной идентификации.")
    add_table(doc, "Таблица 5.2 - Канонические результаты SIL-конвейера AIR56B2", ["Проверка", "Объём", "Результат", "Ограничение интерпретации"], [
        ["Скалярное V/f", f"{vf_matrix.get('total_trial_count', 0)} режимов", f"{vf_matrix.get('summary', {}).get('passed_trials', 0)}/{vf_matrix.get('total_trial_count', 0)} PASS", "Модельный объект; без физической фазировки"],
        ["Матрица защит", f"{fault_matrix.get('total_fault_case_count', 0)} инъекций", f"{fault_matrix.get('summary', {}).get('passed_fault_cases', 0)}/{fault_matrix.get('total_fault_case_count', 0)} PASS", "Не измеряет задержку аппаратного break"],
        ["Encoder-observer FOC", f"{encoder_foc.get('sample_count', 0)} варианта", f"{encoder_foc.get('summary', {}).get('passed_trials', 0)}/{encoder_foc.get('sample_count', 0)} PASS", "Параметры двигателя не измерены"],
        ["Оптимизация потерь", f"{loss_study.get('case_count_comparable', 0)} точек", f"медиана {loss_study.get('summary', {}).get('loss_saving_pct_median', 0.0):.2f} %", "Расчётные, а не измеренные потери"],
        ["Sensorless plant", "15, 30 и 45 Гц", f"средняя ошибка {sensorless_study.get('validation_summary', {}).get('mean_relative_speed_error_pct', 0.0):.2f} %", "5 Гц не подтверждены"],
        ["Нейронная политика", f"{policy_benchmark.get('holdout_summary', {}).get('case_count', 0)} holdout-точек", f"медиана {policy_benchmark.get('holdout_summary', {}).get('loss_saving_vs_fixed_pct_median', 0.0):.2f} %", "Supervised distillation, не hardware PPO"],
    ], [2300, 1700, 2200, 3438], font_size=9)
    classical = common_benchmark.get("methods", {}).get("classical_optimum", {})
    neural = common_benchmark.get("methods", {}).get("neural_policy", {})
    extremum = common_benchmark.get("methods", {}).get("extremum_search", {})
    guarded_lut = common_benchmark.get("methods", {}).get("guarded_lut", {})
    classical_saving = classical.get("saving_vs_fixed_pct", {})
    neural_saving = neural.get("saving_vs_fixed_pct", {})
    extremum_saving = extremum.get("saving_vs_fixed_pct", {})
    lut_saving = guarded_lut.get("saving_vs_fixed_pct", {})
    add_body(doc, f"Для исключения несопоставимости все стратегии дополнительно пересчитаны на одних и тех же {common_benchmark.get('case_count', 0)} отложенных режимах, сгруппированных по 12 независимым вариантам параметров. Кластерный bootstrap с 2000 повторениями учитывает связанность 50 режимов внутри каждого варианта. Среднее снижение расчётных потерь составило {classical_saving.get('mean', 0.0):.2f} % для классического оптимума, {neural_saving.get('mean', 0.0):.2f} % для нейронной политики, {extremum_saving.get('mean', 0.0):.2f} % для ограниченного поиска экстремума и {lut_saving.get('mean', 0.0):.2f} % для LUT с fallback.")
    add_figure(doc, figs["common_benchmark"], "Рисунок 5.2 - Парное сравнение стратегий на общем holdout")
    add_body(doc, f"Нейронная политика не превосходит классический оптимум: медианный разрыв составляет {neural.get('optimality_gap_pct', {}).get('median', 0.0):.2f} %, максимальный — {neural.get('optimality_gap_pct', {}).get('maximum', 0.0):.2f} %. В {neural.get('worse_than_fixed_count', 0)} из {common_benchmark.get('case_count', 0)} режимов её результат хуже фиксированного потока; худшее изменение равно {neural_saving.get('minimum', 0.0):.2f} %. Следовательно, положительная средняя экономия не является основанием для прямой подачи команды actor: необходимы надзорный контур, оценка области применимости и fallback.")
    add_body(doc, "Ограниченный поиск экстремума практически совпал с расчётным optimum, но в SIL использовал истинное значение функции суммарных потерь как измерительный прокси. На стенде такой сигнал непосредственно отсутствует; его необходимо заменить подтверждённой оценкой входной энергии при одинаковой механической нагрузке. Поэтому этот результат показывает потенциальную верхнюю границу model-free адаптации, а не готовый аппаратный алгоритм.")
    add_body(doc, f"Независимый sensorless-наблюдатель использует только приложенное напряжение и задержанные квантованные токи на отдельной RK4-модели объекта. Для 15, 30 и 45 Гц средняя относительная ошибка скорости составила {sensorless_study.get('validation_summary', {}).get('mean_relative_speed_error_pct', 0.0):.2f} %. Режим 5 Гц исключён из подтверждённой области из-за чувствительности voltage model к смещению тока и ошибке Rs. Кроме того, frozen FOC на единственном паспортном наборе не прошёл робастный holdout, что подтверждает обязательность измерения Rs, Rr, Lls, Llr, Lm и J.")
    add_body(doc, "Допустимая интерпретация каждого уровня доказательств приведена в таблице 5.3.")
    add_table(doc, "Таблица 5.3 - Интерпретация уровней доказательств", ["Уровень", "Что подтверждает", "Чего не подтверждает"], [
    ...
    ], [1900, 3750, 3988], font_size=10)

    doc.add_heading("5.3 Последовательность HIL- и стендовых испытаний", level=2)
    add_body(doc, "Последовательность этапов и запрет перехода через незакрытый gate представлены на рисунке 5.3.")
    add_figure(doc, figs["test_steps"], "Рисунок 5.3 - Этапы экспериментальной верификации с критериями перехода")
    ...
    add_body(doc, "Основные сценарии, воздействия, метрики и число повторов приведены в таблице 5.4; форма первичного протокола дана в приложении В.")
    add_table(doc, "Таблица 5.4 - Основные экспериментальные сценарии", ["Сценарий", "Воздействие", "Основные метрики", "Объём серии"], [
    ...
    add_body(doc, "Gate-критерии сведены в таблицу 5.5. Чек-лист силового допуска приведён в приложении Г, а связь задач с доказательствами — в приложении Д.")
    add_table(doc, "Таблица 5.5 - Gate-критерии дальнейших работ", ["Переход", "Обязательные условия"], [
    ...
    add_list(doc, [
        ...
        f"канонический manifest v6 прошёл {sum(bool(value) for value in manifest_gates.values())}/{gate_count} программных ворот, однако hardware_release_ready намеренно остаётся false;",
        f"на едином holdout нейронная политика дала среднее снижение расчётных потерь {neural_saving.get('mean', 0.0):.2f} %, но ухудшила baseline в {neural.get('worse_than_fixed_count', 0)} режимах, поэтому активный actor требует supervisor и fallback;",
        "воспроизводимое SIL-моделирование является подготовительным уровнем доказательств и не подменяет физический эксперимент."], numbered=True)
...
def add_conclusion(doc: Document) -> None:
    ...
    neural = common.get("methods", {}).get("neural_policy", {})
    neural_saving = neural.get("saving_vs_fixed_pct", {})
    ...
    add_list(doc, [
        "подготовлены прослеживаемая программная конфигурация и выпущенная многостраничная схема подключения NUCLEO-G431RB — X-NUCLEO-IHM09M2 — STEVAL-IPM15B — АИР56В2 с верхним уровнем UNO Q; физическая платформа ещё не прошла HIL-приёмку;",
        ...
        "программно реализован 32-байтовый UART-контракт версии 0x02 и разработана схема прямого подключения внутри общего HOT-домена; физический тракт ещё не собран и не проверен;",
        ...
        f"сформирован SIL-конвейер с ансамблем из 256 вариантов и manifest v6: пройдено {sum(bool(value) for value in gates.values())}/{len(gates)} программных ворот при hardware_release_ready=false;",
        f"на holdout из {common.get('case_count', 0)} режимов нейронная политика дала среднее снижение расчётных потерь {neural_saving.get('mean', 0.0):.2f} % и медианный разрыв от оптимума {neural.get('optimality_gap_pct', {}).get('median', 0.0):.2f} %, но в {neural.get('worse_than_fixed_count', 0)} режимах ухудшила baseline, что обосновывает supervisor и fallback;",
        "для sensorless-наблюдателя подтверждена модельная область 15-45 Гц; 5 Гц и frozen FOC по одному паспортному набору отвергнуты;",
    ...
    add_body(doc, "Дальнейшая работа включает HIL-проверку защит, калибровку каналов, определение Rs, Rr, Lls, Llr, Lm и J, испытания V/f и FOC, затем ИИ в shadow mode и ограниченном активном режиме. Ослабление поля выше 50 Гц выделяется в отдельный опыт после механического и теплового допуска. Итоговые положения для защиты должны опираться на статистически подтверждённый стендовый результат; до него сохраняется различие между расчётным, программно проверенным и экспериментальным доказательством.")
...
def add_illustrative_material(doc: Document) -> None:
    ...
    figures = [
        ...
        "Рисунок 5.2 — Парное сравнение стратегий на общем holdout",
        "Рисунок 5.3 — Этапы экспериментальной верификации с критериями перехода",
    ...
    tables = [
        ...
        "Таблица 5.2 — Канонические результаты SIL-конвейера AIR56B2",
        "Таблица 5.3 — Интерпретация уровней доказательств",
        "Таблица 5.4 — Основные экспериментальные сценарии",
        "Таблица 5.5 — Gate-критерии дальнейших работ",
        "Таблица А.1 — Содержимое зафиксированного расчётного профиля",
        "Таблица Б.1 — Прямое соединение UART UNO Q — Nucleo",
...
def add_appendices(doc: Document, figs: dict[str, Path]) -> None:
    ...
    add_table(doc, "Таблица Б.1 - Прямое соединение UART UNO Q — Nucleo", ["Сигнал", "UNO Q", "NUCLEO-G431RB", "Условие"], [
        ["Команда в привод", "D1 / PB6 / Serial1 TX", "PB7 / USART1_RX", "прямой UART 3,3 В"],
        ["Ответ", "D0 / PB7 / Serial1 RX", "PB6 / USART1_TX", "прямой UART 3,3 В"],
        ["Опорный потенциал", "GND UNO Q", "GND Nucleo", "общая HOT_GND = J7 DC−"],
        ["Питание 3,3 В", "не соединять с Nucleo", "не соединять с UNO Q", "каждая плата от своего источника"],
        ["Программный ESTOP", "flags=0x02", "MC_StopMotor1(); latch fault", "не заменяет аппаратный E-stop"],
        ["Рабочий внешний интерфейс", "Wi-Fi", "проводов наружу нет", "USB/ST-Link только без силовой части"],
    ], [2200, 2500, 2500, 2438], font_size=9)
    ...
    add_table(doc, "Таблица Д.1 - Связь задач, программных объектов и подтверждений", ["Задача", "Объект проекта", "Текущее доказательство", "Будущее доказательство"], [
        ...
        ["FOC", "Encoder-observer FOC + MCSDK stage S2", "SIL 24/24; frozen nameplate FOC отклонён", "S4 после аппаратной ID"],
        ["ИИ-адаптация", "Policy, LUT и common benchmark", "600 holdout-точек; supervisor обязателен", "Shadow + парный A/B"],
