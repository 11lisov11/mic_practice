...
### ПК (через ADB)
Только сервисный режим: сеть, выпрямитель и STEVAL J7 физически отключены, DC-link измеренно разряжен. При силовой работе этот тракт запрещён.
1. `adb devices`
...
Этот legacy-мост оставлен только для низковольтной разработки без силовой части. Рабочий HV-режим использует прямой Wi-Fi телефона к UNO Q и не проходит через ПК.
...
```
python tools/adb_deploy_web_hmi.py --device <ADB_ID> --restart --confirm-service-mode
```
...
```powershell
py -3 -u tools\adb_deploy_web_hmi.py --device <ADB_ID> --restart --standalone-hv --confirm-service-mode
```

Один сервер предоставляет два переключаемых по Wi-Fi профиля: `HV 310 В` и `LV TEST`. USB после установки для переключения не нужен. Смена профиля доступна только в `SAFE`; сервер отправляет `STOP`, снимает текущее разрешение и проверяет свежий статус STM32. После перезапуска UNO Q всегда выбирается исходный fail-closed профиль `HV 310 В`.

В `LV TEST` необходимо ввести `ARM LV HV OFF`. Профиль допускается только при физически отключённой силовой шине и телеметрии `0..10 В`; каждый `START` автоматически ограничивается 3 секундами. В `HV 310 В` требуется `ARM 310V`, а телеметрия шины должна находиться в диапазоне `100..400 В`. Разрешение выдаётся на 30 секунд только при живой связи с Nucleo, состоянии `SAFE`, выключенном PWM, отсутствии `ESTOP`, fault и ошибок UART, свежей телеметрии и правдоподобной температуре радиатора. Внешний сетевой soft-start работает автономно перед выпрямителем и не управляется Nucleo. После `START` Nucleo ждёт `Vbus 250...385 В` непрерывно не менее 3,5 с и только затем запускает MCSDK; HMI подтверждает `softstart_ready` и фактический `PWM_ACTIVE`. Ручная команда `PRECHARGE` запрещена. Это программное ограждение не заменяет внешний E-STOP, аппаратный shutdown IPM, предохранитель, проверенный soft-start и закрытый корпус.

Матрица UNO Q показывает заданную частоту ещё до запуска. При работе она чередует частоту и расчётные обороты; нижние три пикселя обозначают экран оборотов. Два пикселя слева показывают устойчивую шину после автономного soft-start, два пикселя справа - фактический `PWM_ACTIVE` MCSDK. На веб-странице частота и обороты отображаются одновременно.
...
py -3 -u tools\adb_deploy_web_hmi.py --device <ADB_ID> --restart --standalone-hv `
  --firmware-update-token-local-file .unoq_firmware_update_token --enable-network-ssh --confirm-service-mode
```
...
AS5600 в архивном Blue Pill-тракте (не подключать к UART Nucleo одновременно):
- `PB10` = `I2C2_SCL`
- `PB11` = `I2C2_SDA`
- `3.3V` и `GND`
...
- `PRECHARGE ON|OFF` - не поддерживается: внешний soft-start автономен, а `PB4` свободен
- `PFC ON|OFF`
...
Поля `precharge` и `bp_ext & 0x08` должны оставаться нулевыми всегда. Готовность автономного soft-start передаётся отдельно как `bp_softstart_ready`; HMI ждёт её вместе с реальным `PWM_ACTIVE`.

Матрица UNO Q показывает заданную частоту ещё до `START`. Верхняя левая отметка означает `bp_softstart_ready`, верхняя правая означает активный PWM; после `STOP` правая должна погаснуть.
...
Low-voltage regression должен подтверждать `precharge=0` и `(bp_ext & 0x08)=0` во всех состояниях. Отдельная HIL-проверка должна подтвердить ограничение пускового тока внешним модулем, его внутреннее шунтирование NTC, порог готовности 250 В, выдержку 3,5 с и сброс готовности при падении Vbus ниже 200 В.
...
telemetry даже с `--allow-hv`. Двенадцатая сохраняет совместимость старого
NTC/PRECHARGE tooling; ручное управление отклоняется, поскольку внешний soft-start
автономен и управляющего GPIO нет. Тринадцатая
...
> Архив для воспроизводимости старых экспериментов. Не использовать для сборки Nucleo-стенда. Актуальные схема, распиновка и команды находятся в корневом `README.md` и `output/pdf/MIC_AI_NUCLEO_SYSTEM_WIRING.pdf`.
