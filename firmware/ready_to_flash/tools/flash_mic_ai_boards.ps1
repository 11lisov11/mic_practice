...
if ([string]::IsNullOrWhiteSpace($PackageDirectory)) {
    $standalonePackage = [System.IO.Path]::GetFullPath((Join-Path $scriptRoot ".."))
    if (Test-Path -LiteralPath (Join-Path $standalonePackage "flash-package-manifest.json") -PathType Leaf) {
        $PackageDirectory = $standalonePackage
    } else {
...
if ($targets -contains "Linux") {
    $deployArgs = @("-3", $linuxDeploy, "--restart", "--$LinuxProfile", "--confirm-service-mode")
