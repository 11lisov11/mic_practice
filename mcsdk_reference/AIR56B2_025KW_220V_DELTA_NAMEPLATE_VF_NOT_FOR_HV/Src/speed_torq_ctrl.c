...
/**
  * @brief Set the primary speed sensor and save the former primary to the auxiliary speed sensor
  * @param pHandle: handler of the current instance of the SpeednTorqCtrl component.
  * @param  error: represents the delta angle between primary and auxiliary sensor
  * @param  margin: represents the maximum delta allowed to perform the sensor switch
  * @param sensor: new primary sensor used to execute FOC algo
  * @retval true if command succeed, false otherwise
  *
  *
  */
bool STC_SensorSet(SpeednTorqCtrl_Handle_t *pHandle, int16_t error, uint16_t margin, SpeednPosFdbk_Handle_t *sensor)
{
  bool status = false;
#ifdef NULL_PTR_CHECK_SPD_TRQ_CTL
  if (MC_NULL == pHandle)
  ...
  {
#endif
  SpeednPosFdbk_Handle_t *tempSPD;
  uint16_t absError;

  if (pHandle->SPD == sensor ) /* sensor already set */
  {
    status = true;
  }
  else {
    if (pHandle->Mode != MCM_POSITION_MODE)
	{
      if (pHandle->AUXSPD == sensor )
      {
        absError = (error >=0) ? error : -error;
        if (absError < margin) {
          tempSPD=pHandle->SPD;
          __disable_irq();
          pHandle->SPD = pHandle->AUXSPD;
          pHandle->AUXSPD = tempSPD;
          __enable_irq();
          status = true;
        }
        else {
          /* Noting to do, primary and auxiliary not in sync */
        }
      }
      else {
        /* target sensor is not primary nor auxiliary */
      }
    }
    else {
    /* Mode == MCM_POSITION_MODE nothing to do return false*/
    }
  }
#ifdef NULL_PTR_CHECK_SPD_TRQ_CTL
  }
