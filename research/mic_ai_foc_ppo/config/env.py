...
class ScalarVfParams:
    ...
    f_min: float
    f_max: float
    # The simulation uses phase-to-neutral quantities.  Keep this value in
    # the selected motor profile instead of reading a module-wide nameplate.
    u_phase_nom: float | None = None
...
class EnvConfig:
    ...
    # Explicit mechanical-speed normalization base for training/evaluation.
    # It must come from the selected motor profile, never from a hard-coded
    # electrical frequency hidden inside the training script.
    omega_base_rad_s: float | None = None
# ---------------------------------------
...
def create_default_env() -> EnvConfig:
    return EnvConfig(
        ...
        scalar_vf=ScalarVfParams(
            ...
            f_min=1.0,
            f_max=50.0,
            u_phase_nom=NAMEPLATE_U_LL / math.sqrt(3.0),
        ),
        foc=FocParams(
            kp_id=1.0,
            ki_id=100.0,
        ...
        omega_base_rad_s=2.0 * math.pi * NAMEPLATE_N_RATED / 60.0,
    )
