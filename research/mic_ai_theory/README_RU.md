...
Этап 10.09.2026: выбор следующей пробы по ожидаемой энергетической ценности.
Уравнения: `../AIR56B2_ENERGY_VALUE_METHOD_RU.md`; замороженный протокол:
`../AIR56B2_ENERGY_VALUE_PROTOCOL_RU.md`. Отдельное расширение малой нагрузки:
`../AIR56B2_ENERGY_VALUE_LIGHT_LOAD_PROTOCOL_RU.md`. Команда полного воспроизведения
из рабочего репозитория: `powershell -ExecutionPolicy Bypass -File tools/run_air56b2_energy_value.ps1`.
Это цифровые исследования поверх неизменного FOC, не новая готовая прошивка.

Продолжение 09.09.2026: выбор парных проб по информации о решении и
эмпирической цене полного цикла. Протокол: `../AIR56B2_ACTIVE_PROBE_PROTOCOL_RU.md`;
близкие аналоги: `../AIR56B2_ACTIVE_PROBE_PRIOR_ART_RU.md`.
Команда воспроизведения: `powershell -ExecutionPolicy Bypass -File tools/run_air56b2_active_probe.ps1`.
Результат 224 опытов и 21 уточнения: `../AIR56B2_ACTIVE_PROBE_RESULTS_RU.md`;
активный выбор не дал преимущества, обучение цены не изменило траектории.
Это проверка гипотезы, не готовая нейропрошивка и не заявленная новизна.

Этап 09.09.2026: проверка энергетического бюджета проб и окупаемости с
ограниченным временем возврата. Протокол: `../AIR56B2_BUDGET_PROTOCOL_RU.md`;
ближайшие аналоги: `../AIR56B2_NOVELTY_AUDIT_20260909_RU.md`.
Это исследовательский фильтр известных методов, без установленной новизны,
без нового обучения ИИ и без изменения прошивок. Воспроизведение:
`powershell -ExecutionPolicy Bypass -File tools/run_air56b2_probe_budget.ps1`
из корня основного проекта.

Новый динамический этап 08.09.2026: физическая ветвь потерь стали,
проверка дискретного баланса энергии и адаптация потока с учётом затрат проб.
Результаты: `../AIR56B2_DYNAMIC_POWER_RESULTS_RU.md`;
уравнения: `../AIR56B2_ENERGY_CORE_THEORY_RU.md`.
Это отдельный усреднённый эксперимент, не новая готовая прошивка.
...
Обновление 08.09.2026: добавлены `air56b2_measured_loss_fit`,
`air56b2_measurement_context` и новый эксперимент по адаптации потока на основе
измерительного окна. Он включает исправленную интерпретацию момента на валу
и отражённое роторное рассеяние. Старые проценты экономии в модели потерь
требуют пересчёта; исторические файлы сохранены. Отчёт и теория:
`../AIR56B2_MEASUREMENT_ADAPTATION_RESULTS_RU.md`,
`../AIR56B2_MEASUREMENT_ADAPTATION_THEORY_RU.md`.
