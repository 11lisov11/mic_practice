...
def is_volatile_snapshot_path(root: Path, path: Path) -> bool:
    """Ignore interpreter caches: they are generated by a read-only test run."""
...
def build_manifest(root: Path) -> dict[str, object]:
    ...
    for path in sorted(root.rglob("*")):
        if not path.is_file() or path.name == MANIFEST_NAME or is_volatile_snapshot_path(root, path):
            continue
        files.append(
            {
...
def verify_manifest(root: Path) -> dict[str, object]:
    ...
    actual_paths = {
        ...
        if path.is_file() and path.name != MANIFEST_NAME and not is_volatile_snapshot_path(root, path)
    }
